...
$registros=mysqli_query($conexion,"select aclaracion from preguntas_juego where id_pregunta='$pregunta'") or die("Problemas en el select:".mysqli_error($conexion));
	...
	{		$r=mysqli_query($conexion,"select monedas from seguimiento_juego where usuario='$usuario'") or die("Problemas en el select:".mysqli_error($conexion));
...
.then((value) => {
		  swal("Felicidades!", "Ganaste 100 monedas, recuerda que con 200 monedas ganas un PILAR", "warning")
		.then((value) => {
		 correcto.play();
		   p1();
		})
